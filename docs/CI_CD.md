...
Với mỗi pull request mới, chủ động gọi `@coderabbitai full review`. Sau commit mới, gọi `@coderabbitai review` nếu review tăng dần tự động bị bỏ qua. Trạng thái skipped, rate-limited hoặc chỉ có summary không chứng minh đã có full review. CodeRabbit có thể tự đánh dấu kết quả review là approve hoặc request changes; trạng thái đó không tự quyết định việc merge. Coordinator đánh giá phát hiện và quyết định merge sau khi đủ bằng chứng; chỉ sửa vấn đề có căn cứ, không áp dụng gợi ý máy móc. Không bật cơ chế để CodeRabbit tự commit bản sửa lỗi, tự giải quyết hội thoại hoặc tự merge. Sau sửa đổi, chạy lại CI và xác nhận review bao phủ commit mới nhất trước khi merge.
